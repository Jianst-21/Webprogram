// form.js — Validasi form & AJAX submit (CREATE)
...
$(document).ready(function () {
  ...
  $('#studentForm').validate({
    ...
    errorPlacement: function (error, element) {
      if (element.parent().hasClass('input-group')) error.insertAfter(element.parent());
  ...
  function submitForm() {
    ...
    $.ajax({
      ...
      error: function (xhr) {
        const msg = xhr.responseJSON ? xhr.responseJSON.error : 'Terjadi kesalahan pada server';
        toastr.error(msg);
        $('#formAlertArea').html(
      ...
      complete: function () {
        $btn.html('<i class="fas fa-floppy-disk me-2"></i>Simpan').prop('disabled', false);
      }
    });
  }

  // ============================
  // Tombol Pratinjau (jQuery slideDown + animate)
  // ============================
  $('#btnPreview').on('click', function () {
    ...
    const status   = $('input[name="status"]:checked').val() || 'Aktif';

    const html = `
      <div class="col-6 col-md-3">
        <div class="preview-item-label">NIM</div>
        <div class="preview-item-val">${nim}</div>
      </div>
      <div class="col-6 col-md-5">
        <div class="preview-item-label">Nama</div>
        <div class="preview-item-val">${nama}</div>
      </div>
      <div class="col-12 col-md-4">
        <div class="preview-item-label">Email</div>
        <div class="preview-item-val" style="font-size:0.8rem">${email}</div>
      </div>
      <div class="col-6 col-md-4">
        <div class="preview-item-label">Program Studi</div>
        <div class="preview-item-val">${prodi}</div>
      </div>
      <div class="col-6 col-md-2">
        <div class="preview-item-label">Angkatan</div>
        <div class="preview-item-val">${angkatan}</div>
      </div>
      <div class="col-6 col-md-2">
        <div class="preview-item-label">Semester</div>
        <div class="preview-item-val">Sem-${semester}</div>
      </div>
      <div class="col-6 col-md-2">
      ...
      <div class="col-6 col-md-2">
        ...
        <div class="preview-item-val">${status}</div>
      </div>`;
  ...
  // ============================
  // Real-time input filtering (jQuery)
  // ============================
  ...
  // Highlight input-group-text icon on focus
